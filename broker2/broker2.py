...
def send_ads():
    ...
    cur.execute('SELECT email from subscriptions where email NOT IN (SELECT email FROM subscriptions WHERE topic = %s AND port = %s) ',(topic,'6002'))
    emailids_from_6002 = cur.fetchall()

    _6001 = requests.get('http://broker1:6001/send_ads_broker', params={'topic':topic})
    emailids_from_6001 = _6001.json()['eid']
    _6003 = requests.get('http://broker3:6003/send_ads_broker', params={'topic':topic})
    emailids_from_6003 = _6003.json()['eid']
...
def get_subs():
    topic = request.args.get('topic')
    print("from broker2 topic name: ",topic)
    config = {
            'user': 'root',
            'password': 'root',
            'host': 'db',
    ...
    cur = connection.cursor()
    x = 0
    cur.execute('SELECT email FROM subscriptions WHERE topic = %s AND port = %s',(topic,'6002'))
    emailids_from_6002 = cur.fetchall()

    _6001 = requests.get('http://broker1:6001/get_eid', params={'topic':topic})
    emailids_from_6001 = _6001.json()['eid']
    _6003 = requests.get('http://broker3:6003/get_eid', params={'topic':topic})
    emailids_from_6003 = _6003.json()['eid']
...
def subber():
    ...
    if (str(topic) == "TheCrewGame" or str(topic) == "FarCrygame" or str(topic) == "assassinscreed"):
        return requests.get('http://broker3:6003/sub', params={'name':name,'email':email,'topic':topic,'port_number':port_number})
...
def unsub():
    ...
    if (str(topic) == "TheCrewGame" or str(topic) == "FarCrygame" or str(topic) == "assassinscreed"):
        return requests.get('http://broker3:6003/unsubber', params={'email':email,'topic':topic})
