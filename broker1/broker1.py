...
def send_ads():
    ...
    cur.execute('SELECT email from subscriptions where email NOT IN (SELECT email FROM subscriptions WHERE topic = %s AND port = %s) ',(topic,'6001'))
    emailids_from_6001 = cur.fetchall()
...
def get_subs():
    ...
    cur = connection.cursor()
    x = 0
    cur.execute('SELECT email FROM subscriptions WHERE topic = %s AND port = %s',(topic,'6001'))
    emailids_from_6001 = cur.fetchall()
